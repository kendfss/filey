...
"""
__all__ = (
    'forbiddens Thing Place Thing Path Address Directory Folder File Library Scanner'
    .split()
)
...
class Thing:
    ...
    def ancestry(self) -> str:
        ...
        for i, anc in enumerate(ancs):
            line = (
                ...
                + ('', '.' * i)[i > 0]
                + i * '  '
                + [i for i in str(anc).split(os.sep) if i][-1]
                + '/'
            )
            print(line)
        return self
    ...
    def delevel(self, steps: int = 1, path: bool = False) -> str:
        ...
        """
        return (
            shell.delevel(self.path, steps)
            if path
            else Place(shell.delevel(self.path, steps))
        )
    ...
    def clone(
        self, folder: str = None, name: str = None, sep: str = '_', touch=False
    ) -> _Path:
        """
        ...
        if not self.exists:
            raise NotImplementedError(
                f"Copying is not implemented for inexistant files/directories"
            )
        elif folder:
...
class File(Thing):
    ...
    def open(
        ...
        """
        fobj = open(
            self.path, mode, buffering, encoding, errors, newline, closefd, opener
        )
        self.__stream = fobj
    ...
    def cat(
        self, text: bool = True, lines: bool = False, copy: bool = False
...
class Place(Thing):
    ...
    def __getitem__(self, item: str | int) -> _Path:
        ...
        raise ValueError(
            f'The folder "{self.name}" does not contain anything called "{item}"'
        )
    ...
    def children(self) -> _Place:
        ...
        return (
            ...
            if (addy := Thing(os.path.join(self.path, i))).isdir
        )
    ...
    def files(self) -> Iterator[File]:
        ...
        return (
            addy.obj
            for i in os.listdir(self.path)
            if (addy := Thing(os.path.join(self.path, i))).isfile
        )
    ...
    def content(self) -> Iterator[Thing]:
        ...
        """
        return tuple(
            Thing(os.path.join(self.path, i)).obj for i in os.listdir(self.path)
        )
    ...
    def branches(self) -> Iterator[_Place]:
        ...
        """
        yield from map(
            lambda x: Thing(x).obj, walking.folders(self.path, absolute=True)
        )
        ...
        return tuple(
            unique(
                filter(
                    ...
                    (
                        File(path).mime
                        for path in self.gather(dirs=False, absolute=True)
                    ),
                )
            )
        )
    ...
    def exts(self) -> tuple[str]:
        ...
        return tuple(
            unique(
                filter(
                    None,
                    (File(path).ext for path in self.gather(dirs=False, absolute=True)),
                )
            )
        )
...
class Scanner:
    ...
    def pattern(self):
        ...
        """
        return re.compile(
            ('|'.join(self.keywords.split()), self.keywords)[self.strict], self.case
        )
    ...
    def __call__(self, path: str, lines: bool = None) -> bool:
        ...
        try:
            with self.opener(path, self.mode) as fob:
                ...
                return self.pattern.search(path) or any(
                    map(self.pattern.search, method())
                )
